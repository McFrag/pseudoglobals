...
static int pseudoglobals_register_name(const char *name, size_t len)
{
    ...
    while (len > 0 && (*name == ' ' || *name == '\t')) { ++name; --len; }
    ...
    if (zend_hash_exists(&PGLOB(registered), key)) {
        php_error_docref(NULL,E_WARNING,
            "Ignoring duplicate pseudoglobal \"%s\"", ZSTR_VAL(key));
    ...
    if (zend_hash_add_empty_element(&PGLOB(registered), key) == NULL) {
        php_error_docref(NULL,E_WARNING,
            "Unable to add pseudoglobal \"%s\" to registry", ZSTR_VAL(key));
        zend_string_release(key);
        return FAILURE;
    }

    if (zend_register_auto_global(key, 0, NULL) == FAILURE) {
        php_error_docref(NULL,E_WARNING,
            "Zend rejected pseudoglobal \"%s\"", ZSTR_VAL(key));
...
{
    const char *start, *p;
    if (PGLOB(register_names) == NULL || *PGLOB(register_names) == '\0') return SUCCESS;
    start = PGLOB(register_names);
    for (p = start; ; ++p) {
        if (*p == ',' || *p == '\0') {
            if (pseudoglobals_register_name(start,(size_t)(p-start)) == FAILURE) return FAILURE;
            if (*p == '\0') break;
            start = p + 1;
        }
    }
...
{
    zend_hash_init(&PGLOB(registered),8,NULL,NULL,1);
