...
static void pseudoglobals_init_globals(zend_pseudoglobals_globals *globals)
{
...
{
    ZEND_INIT_MODULE_GLOBALS(pseudoglobals,pseudoglobals_init_globals,NULL);
    if (pseudoglobals_config_minit(module_number) == FAILURE) return FAILURE;
...
PHP_RINIT_FUNCTION(pseudoglobals)
{
    ...
    if (!pseudoglobals_initialize()) return FAILURE;
    return SUCCESS;
}
...
PHP_MINFO_FUNCTION(pseudoglobals)
{
    ...
    snprintf(registered_count,sizeof(registered_count),"%u",pseudoglobals_registry_count());
    ...
    php_info_print_table_row(2,"Registered names",registered_count);
